(()=>{
 ...
 const cardText=card=>(card.querySelector('.profit-original-notes')?.textContent||'').replace(/^نص الطلب الأصلي:\s*/,'');
 async function apply(card,generation){
  ...
  const text=cardText(card),token=`${generation}:${card.dataset.id||''}:${text}`;card.dataset.profitAnalysisToken=token;
  const result=await(rr.calculateCostSmart?.(text)||rr.calculateCost(text));
  // A semantic request may finish after the report was redrawn or the order text
  // changed. Such a result must never overwrite the current card.
  if(generation!==requestedGeneration||!card.isConnected||card.dataset.profitAnalysisToken!==token||cardText(card)!==text||state?.view!=='daily-profits')return;
  if(card.dataset.manualCostOverride==='1'||card.dataset.manualCost==='1'||card.dataset.status==='partial'){clearIncomplete(card);return}
  card.dataset.profitEngineVersion=String(result.engineVersion||rr.version||'central-v13');card.dataset.profitEngineSource=String(result.source||'central');
 ...
 let running=false,rerun=false,requestedGeneration=0;
 async function run(){
  const generation=++requestedGeneration;
  if(running){rerun=true;return}if(state?.view!=='daily-profits'||!rules())return;
  running=true;
  try{await rules().loadRemote?.();for(const c of document.querySelectorAll('.profit-order-card')){if(generation!==requestedGeneration)break;await apply(c,generation)}}
  finally{running=false;if(rerun){rerun=false;run()}}
 }
