(()=>{
 ...
 async function render(force=false){
  ...
  try{
   ...
   const cards=[...document.querySelectorAll('.profit-order-card')];await Promise.all(cards.map(async c=>{try{const o=(await api('/orders/'+c.dataset.id)).order;c.dataset.settlementId=String(o?.delivery_company_settlement_id||'')}catch{c.dataset.settlementId=''}}));
 ...
 document.addEventListener('change',e=>{if(e.target?.id==='profitDate'||e.target?.id==='profitStore'){lastKey='';setTimeout(()=>render(true),50)}});document.addEventListener('click',e=>{if(e.target?.id==='loadProfits'){lastKey='';setTimeout(()=>render(true),150)}});new MutationObserver(()=>{removeAll();setTimeout(()=>render(false),120)}).observe(document.documentElement,{childList:true,subtree:true});setInterval(()=>{removeAll();render(false)},1200);render(true);
})();