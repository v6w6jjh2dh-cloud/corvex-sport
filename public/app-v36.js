...
function parseSmart(text){
  ...
  // 6) الملاحظات المرتبة — V32: لا نسمح بضياع أي سطر من تفاصيل الطلب
  const noteRows = [];
  ...
  // الهاتف الأساسي له خانته الخاصة، فلا نكرره في الملاحظات.
  // إذا وُجد رقم ثانٍ أو أكثر فقط، نحفظ الأرقام الإضافية في الملاحظات.
  extraPhones.forEach((p,idx)=>{
    pushNote(idx===0?'هاتف إضافي':'هاتف إضافي '+(idx+1),p);
  });
  ...
  for(let i=0;i<lines.length;i++){
    ...
    // الاسم فقط هو الذي لا يعاد في الملاحظات
    if(name!=='لا يوجد' && line===name)continue;

    // الألوان لها أولوية مطلقة قبل فحص العنوان أو المنطقة
    if(isColorLine(line)){
      pushNote('اللون',line);
      continue;
    }

    // السعر له خانة مستقلة، فلا نكرره في الملاحظات.
    if(isPriceLine(line)){
      continue;
    }
    ...
    // لا نكرر سطر المنطقة/العنوان الحقيقي فقط
    if(pa.area){
      continue;
    }

    // أي سطر عنوان إضافي تم نقله للعنوان لا نكرره
    if(used.has(i) && containsAny(normalizeArabic(line),ADDRESS_WORDS)){
      continue;
    }

    const item=classifyNoteLine(line);
    pushNote(item.label,item.value);
  }

  // شبكة أمان أخيرة: أي سطر لم يظهر في الاسم/الهاتف/العنوان/السعر ولا في الملاحظات يضاف كملاحظة.
  // هذا يمنع ضياع ألوان أو تفاصيل جديدة لم يتعلمها القاموس بعد.
  const notesTextNormalized=()=>normalizeArabic(noteRows.join(' '));
  for(let i=0;i<lines.length;i++){
    const line=lines[i];
    if(phoneFrom(line))continue;
    if(name!=='لا يوجد' && line===name)continue;
    ...
    const pa=splitAreaAddress(line);
    if(pa.area)continue;
    if(used.has(i) && containsAny(normalizeArabic(line),ADDRESS_WORDS))continue;

    const nl=normalizeArabic(line);
    if(nl && !notesTextNormalized().includes(nl)){
...
async function newOrder(){
  ...
  c.innerHTML=`<div class="page-title"><div><h1>إضافة طلب</h1><div class="sub">Smart Parser V36 • Clean Notes • الصق الطلب كامل أو عبّئ الحقول يدويًا</div></div></div>
  <div class="card">
