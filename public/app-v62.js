...
const PRIORITY_LOCAL_ALIASES = [
  // اسم شارع داخل عمان، وليس إشارة إلى محافظة مأدبا.
  ['عمان',['شارع مادبا']],
  // المشقر تتبع مأدبا تشغيليًا، حتى إذا وردت كلمة عمان في نفس سطر العنوان.
  ['مأدبا',['المشقر']],
...
function code128Svg(value){
  ...
  for(const code of codes){
    ...
    [...pattern].forEach((width,index)=>{const w=Number(width);if(index%2===0)bars+=`<rect x="${x}" y="4" width="${w}" height="46"/>`;x+=w});
  }
  const total=x+18;
