(()=>{
 ...
 async function openManager(prefill=''){
  ...
  const overlay=document.createElement('div');overlay.id='profitModelManagerOverlay';overlay.className='modal-backdrop';overlay.innerHTML=`<div class="modal-card profit-model-manager"><div class="modal-head"><div><h3>إدارة الموديلات والكوست</h3><div class="sub">عرّف الموديل مرة واحدة ليُقرأ في كل الطلبات.</div></div><button type="button" class="btn btn-soft" id="closeProfitModels">✕</button></div>${formHtml(prefill)}<div id="profitModelList"><div class="empty">جاري تحميل الموديلات...</div></div></div>`;document.body.appendChild(overlay);
  ...
  let data;try{data=await api('/profit-models')}catch(e){toast(e.message);close();return}let models=data.models||[];renderList(models);
