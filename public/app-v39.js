...
const PRIORITY_LOCAL_ALIASES = [
  ['الزرقاء',['ضاحية الأمير حسن','ضاحية الامير حسن','الازرق الشمالي','الأزرق الشمالي','جبل فيصل','عوجان الرصيفه','عوجان الرصيفة','الرصيفه','الرصيفة']],
  ...
  ['جرش',['مخيم غزة','مخيم غزه','مخيم جرش']],
  ['عمان',['الدوار الثامن','دوار الثامن','الثامن']],
  ['عمان',['الدوار السابع','دوار السابع','السابع']],
  ['عمان',['الدوار السادس','دوار السادس','السادس']],
  ['عمان',['الدوار الخامس','دوار الخامس','الخامس']],
  ['عمان',['الدوار الرابع','دوار الرابع','الرابع']],
  ['عمان',['الدوار الثالث','دوار الثالث','الثالث']],
  ['عمان',['الدوار الثاني','دوار الثاني','الثاني']],
  ['عمان',['الدوار الأول','دوار الاول','دوار الأول','الاول','الأول']]
];
...
function hasStrongAddressCue(line){
  const n=normalizeArabic(String(line||''));
  return /^(?:سكان|ساكن|ساكنه|ساكنة|عنواني|العنوان|عنوان)\b/.test(n) ||
    containsAny(n,ADDRESS_WORDS);
}
...
function isLikelyName(line,index){
  ...
  // Known first names are authoritative unless the line literally starts as an address.
  const startsAddress=/^(?:سكان|ساكن|ساكنه|ساكنة|عنواني|العنوان|عنوان|المكان)\b/.test(n);
  if((COMMON_NAMES.has(ws[0])||ws[0]==='عيد'||ws[0]==='اسمهان')&&!startsAddress)return true;

  if(hasStrongAddressCue(raw))return false;
  if(containsAny(n,PRODUCT_WORDS))return false;
  if(isColorLine(raw)||isQuantityLine(raw)||isHeightLine(raw))return false;
  if(/وزن|الوزن|وزني|مقاس|المقاس|قياس|لون|اللون|الوان|الطول|طولي|ترند|موديل/.test(n))return false;
  if(priorityLocalMatch(raw)||explicitGovernorateMatch(raw)||shippingAliasMatch(raw))return false;
...
function parseSmart(text){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    const explicitLocation=!!(priorityLocalMatch(line)||explicitGovernorateMatch(line)||shippingAliasMatch(line)||containsAny(n,ADDRESS_WORDS));

    // Product/detail/person lines can never become addresses from a weak generic-place match.
    if(containsAny(n,PRODUCT_WORDS) && !explicitLocation)continue;
    if(isLikelyName(line,i) && !explicitLocation)continue;
...
async function newOrder(){
  ...
  c.innerHTML=`<div class="page-title"><div><h1>إضافة طلب</h1><div class="sub">Smart Parser V39 • Hardened • الصق الطلب كامل أو عبّئ الحقول يدويًا</div></div></div>
  <div class="card">
