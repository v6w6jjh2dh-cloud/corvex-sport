(()=>{
 ...
 function unknownCandidates(text=''){
  const lines=String(text).split(/\n+/).map(line=>normalize(line)).filter(Boolean),unknown=[];
  for(const line of lines){
   ...
   for(let i=0;i<quantities.length;i++){
    ...
    if(!candidate||findAll(candidate).length)continue;
    const words=candidate.split(/\s+/).filter(word=>!COLORS.has(word)&&!['لون','الوان','قطعه','قطع','حبه','حبات','لارج','ميديوم','سمول','xl','xxl','xxxl'].includes(word));
    candidate=words.join(' ').trim();
    if(candidate.length<2||STOP_CONTEXT.test(candidate)||/^\d+$/.test(candidate))continue;
