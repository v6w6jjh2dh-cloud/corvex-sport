(()=>{
  ...
  async function editFastReturn(orderCode,onDone){
    try{
      ...
      <div class="actions"><button id="saveFastRet" class="btn btn-accent">حفظ التعديل</button><button class="btn btn-soft close-fast-ret">إلغاء</button></div></div>`;
      ...
      $('#saveFastRet').onclick=async()=>{
        const items=parseItemsText($('#frItems').value);if(!items.length)return toast('أضف صنفًا واحدًا على الأقل');
        const b=$('#saveFastRet');b.disabled=true;
        try{await fastApi('',{method:'PUT',body:JSON.stringify({order_code:orderCode,return_type:$('#frType').value,reason:$('#frReason').value,notes:$('#frNotes').value,items})});toast('تم تعديل المرتجع');overlay.remove();if(onDone)await onDone()}catch(e){b.disabled=false;toast(e.message)}
      };
  ...
  function fastDashboardHtml(d){
    ...
    <div><h3>سجل الاستلام</h3>${history.length?`<div class="return-history-list">${history.map(r=>`<div class="return-history-row"><div><b>#${r.order_code} — ${esc(r.store_name||'')}</b><div class="sub">${r.return_type==='full'?'مرتجع كامل':'مرتجع جزئي'} • ${(r.items||[]).map(i=>`${esc(i.item_name)} × ${i.quantity}`).join('، ')}</div><div class="return-history-meta">${esc(r.created_by_name||'')} • ${fmtDate(r.created_at)}</div></div><button class="btn btn-soft fast-edit-return" data-code="${r.order_code}">تعديل</button></div>`).join('')}</div>`:'<div class="empty">لا توجد مرتجعات بعد</div>'}</div></div>`;
  }

  async function shadiReturnsView(){
    const c=$('#content');
    c.innerHTML=`<div class="page-title"><div><h1>مركز المرتجعات</h1><div class="sub">اضرب الباركود — يتم تسجيل المرتجع مباشرة بدون تأكيد</div></div><span class="pill return-live-pill">جاهز للمسح</span></div>
    <div class="return-scan-card"><div class="return-scan-title"><span class="return-scan-icon">▥</span><div><h2>مسح سريع</h2><div class="sub">كل ضربة باركود تحفظ المرتجع فورًا.</div></div></div><div class="return-scan-actions"><input id="fastReturnInput" class="input return-code-input" autocomplete="off" placeholder="امسح الباركود"><button id="fastReturnBtn" class="btn btn-primary">تسجيل</button></div><div id="fastReturnStatus"></div></div>
    <div class="card"><div id="fastReturnsDashboard"><div class="empty">جاري التحميل...</div></div></div>`;
    const load=async()=>{try{const d=await fastApi();$('#fastReturnsDashboard').innerHTML=fastDashboardHtml(d);document.querySelectorAll('.fast-edit-return').forEach(b=>b.onclick=()=>editFastReturn(Number(b.dataset.code),load))}catch(e){$('#fastReturnsDashboard').innerHTML=`<div class="empty">${esc(e.message)}</div>`}};
    const scan=async()=>{const inp=$('#fastReturnInput'),code=inp.value.trim();if(!code)return;inp.value='';inp.disabled=true;try{const r=await fastApi('',{method:'POST',body:JSON.stringify({code})});$('#fastReturnStatus').innerHTML=`<div class="return-success"><b>${r.already?'✓ المرتجع مسجل مسبقًا':'✓ تم استلام المرتجع مباشرة'}</b><span>طلب #${r.return_event?.order_code||''}</span></div>`;toast(r.already?'هذا المرتجع مسجل مسبقًا':'تم تسجيل المرتجع');$('#fastReturnsDashboard').innerHTML=fastDashboardHtml(r.dashboard);document.querySelectorAll('.fast-edit-return').forEach(b=>b.onclick=()=>editFastReturn(Number(b.dataset.code),load))}catch(e){toast(e.message)}finally{inp.disabled=false;inp.focus()}};
  ...
  returnsCenterView=async function(){
    ...
    const fastScan=async code=>{
      if(!code)return;
      input.value='';input.disabled=true;
      try{const r=await fastApi('',{method:'POST',body:JSON.stringify({code})});$('#returnOrderPanel').innerHTML=`<div class="return-success"><b>${r.already?'✓ المرتجع مسجل مسبقًا':'✓ تم استلام المرتجع مباشرة'}</b><span>طلب #${r.return_event?.order_code||''} — بدون تأكيد</span>${r.return_event?.order_code?`<button class="btn btn-soft fast-edit-current" data-code="${r.return_event.order_code}">تعديل المرتجع</button>`:''}</div>`;toast(r.already?'هذا المرتجع مسجل مسبقًا':'تم تسجيل المرتجع مباشرة');$('#loadReturns')?.click();document.querySelector('.fast-edit-current')?.addEventListener('click',e=>editFastReturn(Number(e.currentTarget.dataset.code),async()=>$('#loadReturns')?.click()))}catch(e){toast(e.message)}finally{input.disabled=false;input.focus()}
    };
    ...
    const addEdits=()=>document.querySelectorAll('#returnHistory .return-history-row').forEach(row=>{
      if(row.querySelector('.fast-edit-return'))return;const m=row.textContent.match(/#(\d+)/);if(!m)return;const b=document.createElement('button');b.className='btn btn-soft fast-edit-return';b.textContent='تعديل';b.onclick=()=>editFastReturn(Number(m[1]),async()=>$('#loadReturns')?.click());row.appendChild(b);
    });
    const hist=$('#returnHistory');if(hist){new MutationObserver(addEdits).observe(hist,{childList:true,subtree:true});addEdits()}
  };

  function ensureShadiMenu(){
    if(!isShadi())return;const nav=document.querySelector('.side-nav,aside nav,nav');if(!nav||nav.querySelector('[data-view="returns-center"]'))return;
    const b=document.createElement('button');b.dataset.view='returns-center';b.textContent='↩ مركز المرتجعات';b.onclick=()=>show('returns-center');nav.appendChild(b);
  }
