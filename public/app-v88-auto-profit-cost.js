(()=>{
 const MODELS=[
  ...
  {name:'تركي',re:/(?:بنطلون\s*)?تركي/i,price:2.7},{name:'ريبوك',re:/ريبوك/i,price:2.5},{name:'باريس',re:/باريس/i,price:2.5},
  {name:'اولد موني',re:/(?:اولد|أولد|ولد)\s*(?:موني|مني)/i,price:3.5},
  {name:'M',re:/^(?:\d+\s*)?(?:تيشرت|تيشيرت|تشرت|بلوز[هة]?|بلايز)\s*(?:حرف\s*)?(?:m6|m|م6|م|ام6|أم6|ام|أم)\s*$/i,price:3},
 ...
 function qFrom(s=''){
  ...
  m=s.match(/(?:^|\s)(\d+)\s*(?:قطعه|قطع|حبه|حبات|الوان)(?:\s|$)/i);if(m)return Math.max(1,Number(m[1]));
  m=s.match(/(?:^|\s)(\d+)\s+(?=(?:تيشرت|تيشيرت|تشرت|بلوز|بلايز|بنطلون|ريبوك|باريس|اولد|أولد|ولد|جيوب|رياض|قطن|تريكو|بولو))/i);if(m)return Math.max(1,Number(m[1]));
 ...
 function localCost(text=''){
  ...
  for(let i=0;i<lines.length;i++){const line=norm(lines[i]);if(/استرجاع|ارجاع/.test(line))continue;for(const model of MODELS){if(!model.re.test(line))continue;const context=[lines[i-2]||'',lines[i-1]||'',line,lines[i+1]||'',lines[i+2]||''].join(' ');total+=model.price*qFrom(context);found++;break}}
 ...
 async function auto(){
  ...
  for(const card of cards){
   ...
   if(r.found){
    card.dataset.autoCostDone='1';card.dataset.autoCostUnknown='0';card.querySelectorAll('.profit-ai').forEach(b=>b.style.display='none');
    input.value=r.cost.toFixed(2);input.dispatchEvent(new Event('input',{bubbles:true}));
    if(card.dataset.autoCostSaved!=='1'){card.dataset.autoCostSaved='1';const id=Number(card.dataset.id||0),amount=Number(card.querySelector('.profit-amount')?.value||0),fee=Number(card.querySelector('.profit-fee')?.value||0);try{const d=await api('/orders/'+id),o=d.order;if(o&&Math.abs(Number(o.cost_of_goods||0)-r.cost)>.001)await api('/orders/'+id+'/outcome',{method:'PUT',body:JSON.stringify({delivery_status:o.delivery_status,printed:Number(o.printed||0),delivered_amount:amount,delivery_fee:fee,cash_collected:Math.max(0,amount-fee),cost_of_goods:r.cost,delivered_pieces:Number(o.delivered_pieces||0),returned_pieces:Number(o.returned_pieces||0),settlement_note:o.settlement_note||''})})}catch{}}
   }else if(card.dataset.autoCostDone!=='1'){card.dataset.autoCostUnknown='1';const b=card.querySelector('.profit-ai');if(b){b.textContent='✨ موديل غير معروف — تحليل بالذكاء';b.style.display='inline-flex'}}
  }
