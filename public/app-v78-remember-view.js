(()=>{
 ...
 const restorable=new Set(['dashboard','orders','new','deleted-orders','stores','store-add','store-orders','reports','daily-profits','inventory','returns-center','direct-orders','model-performance','permissions','users','regions','couriers','courier-add','courier-settlement','delivery-reconcile','store-account','print','batches','admin-reset','delivery-alerts']);
 ...
 window.__corvexOpenRememberedView=async function(){
  try{
   ...
   if(v==='delivery-alerts'&&typeof window.deliveryAlertsView==='function')await window.deliveryAlertsView();
   else if(v==='model-performance'&&typeof window.modelPerformanceView==='function')await window.modelPerformanceView();
