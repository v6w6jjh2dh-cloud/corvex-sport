(()=>{
 ...
 async function restore(){let tries=0;const timer=setInterval(()=>{tries++;try{const v=localStorage.getItem(KEY);if(state?.user&&v&&restorable.has(v)){clearInterval(timer);if(v==='model-performance'&&typeof window.modelPerformanceView==='function')window.modelPerformanceView();else if(v==='direct-orders'&&typeof window.directOrdersView==='function')window.directOrdersView();else if(v==='admin-reset'&&typeof window.adminResetView==='function')window.adminResetView();else if(typeof show==='function')show(v)}}catch{}if(tries>30)clearInterval(timer)},150)}
