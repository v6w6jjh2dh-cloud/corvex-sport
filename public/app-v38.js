...
const PARSER_INSTRUCTION_WORDS = [
  ...
  'استلام','توصيل','اتصال','اتصل','خصم','بدي','عاوز','اريد','أريد'
].map(normalizeArabic);
...
const PRIORITY_LOCAL_ALIASES = [
  ['الزرقاء',['ضاحية الأمير حسن','ضاحية الامير حسن','الازرق الشمالي','الأزرق الشمالي','عوجان الرصيفه','عوجان الرصيفة','الرصيفه','الرصيفة']],
  ['عمان',['شارع الإذاعة والتلفزيون','شارع الاذاعه والتلفزيون','شارع الاذاعة والتلفزيون','شارع الاذاعه و التلفزيون','شارع الاذاعة و التلفزيون','ناعور','اسكان الكهرباء','إسكان الكهرباء']],
  ['جرش',['مخيم غزة','مخيم غزه']],
  ['عمان',['الدوار الثامن','دوار الثامن','الثامن']],
  ['عمان',['الدوار السابع','دوار السابع']],
  ['عمان',['الدوار السادس','دوار السادس']],
  ['عمان',['الدوار الخامس','دوار الخامس']],
  ['عمان',['الدوار الرابع','دوار الرابع']],
  ['عمان',['الدوار الثالث','دوار الثالث']],
  ['عمان',['الدوار الثاني','دوار الثاني']],
  ['عمان',['الدوار الأول','دوار الاول','دوار الأول']]
];
...
function isLikelyName(line,index){
  ...
  if(isInstructionLine(raw))return false;
  if(containsAny(n,PRODUCT_WORDS))return false;
  if(isColorLine(raw)||isQuantityLine(raw)||isHeightLine(raw))return false;
  if(/وزن|الوزن|وزني|مقاس|المقاس|قياس|لون|اللون|الوان|الطول|طولي/.test(n))return false;

  const ws=n.split(/\s+/).filter(Boolean);
  if(ws.length<2||ws.length>4)return false;

  // If first word is a common given name, strongly prefer person-name interpretation.
  if(COMMON_NAMES.has(ws[0]) || ws[0]==='عيد')return true;

  // Generic Arabic personal-name fallback: reject any confirmed place phrase.
  // findBestPlace is now boundary-safe, so short place aliases cannot match inside surnames.
  if(containsAny(n,ADDRESS_WORDS))return false;
  if(findBestPlace(raw))return false;
...
function classifyNoteLineMulti(line){
  ...
  // Product/garment description always wins over generic detail tokens.
  if(containsAny(n,PRODUCT_CORE_WORDS)){
    out.push({label:'الطلب',value:raw});
    return out;
  }

  const weight=extractWeightValue(raw);
  const height=extractHeightValue(raw);
  ...
  // "3 قطع حرف M" contains both quantity and size; keep both.
  if(/مقاس|المقاس|قياس|\b(?:s|m|l|xl|xxl|xxxl|xxxxl)\b/i.test(raw)){
    out.push({label:'المقاس',value:raw});
  }

  const colors=actualColorsInLine(raw);
  if(colors.length){
    out.push({label:'اللون',value:colors.join(' و ')});
  }

  // "3 ألوان" without actual color names is a detail, not a color value.
...
function parseSmart(text){
  ...
  // 3) Person name. Never ask the location dictionary to classify a confirmed person name.
  for(let i=0;i<lines.length;i++){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    // Extra safety: a line that looks strongly like merchandise is not an address unless it has
    // explicit governorate/priority-local/address wording.
    const strongLocation=!!(priorityLocalMatch(line)||explicitGovernorateMatch(line)||containsAny(n,ADDRESS_WORDS));
    if(containsAny(n,PRODUCT_WORDS) && !strongLocation)continue;
  ...
  if(address!==''){
    ...
    for(let i=0;i<lines.length;i++){
      ...
      if(containsAny(n,ADDRESS_WORDS) && !containsAny(n,PRODUCT_WORDS) && !isLikelyName(lines[i],i)){
        extras.push(lines[i]);
        used.add(i);
      }
    }
  ...
  for(let i=0;i<lines.length;i++){
    ...
    // Structured extraction can intentionally transform weight/color/quantity;
    // only preserve the raw line when none of its meaningful content is represented.
...
async function newOrder(){
  ...
  c.innerHTML=`<div class="page-title"><div><h1>إضافة طلب</h1><div class="sub">Smart Parser V38 • Rebuilt • الصق الطلب كامل أو عبّئ الحقول يدويًا</div></div></div>
  <div class="card">
