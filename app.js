...
function normalizeArabic(v=''){
  return String(v).toLowerCase()
    ...
    .replace(/ئ/g,'ي').replace(/ة/g,'ه').replace(/[ًٌٍَُِّْـ]/g,'')
...
const PRODUCT_WORDS=`
قطعة قطع قطعتين بلايز بلوز بلوزة تيشيرت تيشيرتات تشرت تشيرت بولو تريننغ طقم اطقم بنطلون بناطيل شورت بيجاما جاكيت هودي قميص فستان عباية تنورة جينز ريبوك نايك اديداس بوما زارا مقاس قياس وزن وزني كيلو كغم لون الوان اسود ابيض اخضر ازرق احمر زهري وردي رمادي رصاصي كحلي بني بيج xl xxl xxxl xxxxl ميديوم سمول لارج
`.trim().split(/\s+/).map(normalizeArabic);

const ADDRESS_HINTS=/بجانب|جنب|قرب|مقابل|خلف|امام|شارع|دوار|حي|حاره|اشاره|مسجد|مدرسه|جامعه|مجمع|سوق|مخيم|اسكان|عماره|بنايه|صيدليه|مستشفى|مول|فرع/;
...
function isAmountLine(line){
  ...
  return ((/شامل\s*التوصيل|مع\s*التوصيل|دينار|jod|jd/.test(n)&&/\d/.test(n))
...
function isProductLine(line){
  const n=normalizeArabic(line);
  return PRODUCT_WORDS.some(x=>n.includes(x))
    || /\b(?:s|m|l|xl|xxl|xxxl|xxxxl)\b/i.test(line)
    || (/^\s*\d+\s+/.test(line)&&!isAmountLine(line));
}
function placeMatches(line){
  const n=normalizeArabic(line);
  return PLACE_PHRASES.filter(p=>n===p||n.startsWith(p+' ')||n.includes(' '+p+' '));
}
function isLikelyName(line,index){
  if(!/[\u0600-\u06FF]/.test(line)||/\d/.test(line)||phoneFrom(line)||isAmountLine(line)||isProductLine(line))return false;
  const n=normalizeArabic(line), ws=n.split(/\s+/).filter(Boolean);
  if(ws.length<1||ws.length>4||ADDRESS_HINTS.test(n))return false;
  const known=ws.filter(w=>COMMON_NAMES.has(w)).length;
  if(known>=1 && ws.length<=3)return true; // الاسم الأول معروف + اسم عائلة غير معروف
  if(index<=1 && ws.length===1 && placeMatches(line).length===0)return true;
  return false;
}
function splitPlaceAddress(line){
  const original=String(line).trim(), n=normalizeArabic(original);
  const matches=placeMatches(original);
  if(!matches.length)return {area:'',address:''};
  const p=matches[0];
  const normalizedWords=n.split(/\s+/);
  const placeWords=p.split(/\s+/);
  let start=-1;
  for(let i=0;i<=normalizedWords.length-placeWords.length;i++){
    if(normalizedWords.slice(i,i+placeWords.length).join(' ')===p){start=i;break;}
  }
  const originalWords=original.split(/\s+/);
  if(start<0)return {area:original,address:''};
  const area=originalWords.slice(start,start+placeWords.length).join(' ');
  const before=originalWords.slice(0,start).join(' ');
  const after=originalWords.slice(start+placeWords.length).join(' ');
  return {area,address:[before,after].filter(Boolean).join(' ').trim()};
}

function parseSmart(text){
  const lines=String(text||'').split(/\n+/).map(x=>x.trim()).filter(Boolean);
  const used=new Set();
  let phone='',amount='',name='',area='',address='';

  for(let i=0;i<lines.length;i++){
    const p=phoneFrom(lines[i]);
    if(p){phone=p;used.add(i);break;}
  }
  for(let i=lines.length-1;i>=0;i--){
    if(!used.has(i)&&isAmountLine(lines[i])){amount=amountFrom(lines[i]);used.add(i);break;}
  }

  // الاسم: نعطي أولوية لسطر فيه اسم أول معروف حتى لو اسم العائلة جديد
  for(let i=0;i<lines.length;i++){
    if(!used.has(i)&&isLikelyName(lines[i],i)){
      name=lines[i];used.add(i);break;
    }
  }

  // المنطقة والعنوان: افصل "السلط بجانب جامعة..." إلى السلط + باقي العنوان
  for(let i=0;i<lines.length;i++){
    if(used.has(i)||isProductLine(lines[i]))continue;
    const pa=splitPlaceAddress(lines[i]);
    if(pa.area){
      area=pa.area;
      address=pa.address;
      used.add(i);
  ...
  // أي سطر عنوان إضافي يضاف للعنوان التفصيلي، وليس للملاحظات
  const addressExtra=[];
  for(let i=0;i<lines.length;i++){
    if(used.has(i)||isProductLine(lines[i]))continue;
    const n=normalizeArabic(lines[i]);
    if(ADDRESS_HINTS.test(n)){
      addressExtra.push(lines[i]);used.add(i);
    }
  }
  address=[address,...addressExtra].filter(Boolean).join(' - ');

  // fallback للمنطقة فقط بعد تثبيت الاسم
  if(!area){
    for(let i=0;i<lines.length;i++){
      if(used.has(i)||isProductLine(lines[i]))continue;
      if(/[\u0600-\u06FF]/.test(lines[i])&&!isLikelyName(lines[i],i)){
        area=lines[i];used.add(i);break;
      }
    }
  }

  const notes=lines.filter((_,i)=>!used.has(i)).join(' - ');
  return {name,phone,amount,area,address,notes};
}
async function boot(){
